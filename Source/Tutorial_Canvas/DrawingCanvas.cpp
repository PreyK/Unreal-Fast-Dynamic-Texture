...
UTexture2D* UDrawingCanvas::InitializeCanvas(const int32 pixelsH, const int32 pixelsV, const int32 regionSplit = 4)
{
	...
	{
		RegionSplits = 4;
		FString logtext = FString("Region split ") + FString::FromInt(regionSplit) + FString(
			" is not Power Of 2!, defaulting to 4 splits");
		if (GEngine)
...
void UDrawingCanvas::DrawDot(const int32 pixelCoordX, const int32 pixelCoordY, int brushRadius)
{
	radius = brushRadius;
	brushBufferSize = radius * radius * 4 * bytesPerPixel; //2r*2r * bpp
	canvasBrushMask = std::unique_ptr<uint8[]>(new uint8[brushBufferSize]);
	...
	if (!UpdateFullRegion)
	{
		...
		//log stuff
		FString logtext = "Writing to region " + FString::FromInt(
			RegionLookup[pixelCoordX / CellSize][pixelCoordY / CellSize]);
		if (GEngine)
...
void UDrawingCanvas::UpdateCanvas()
{
	...
	else
	{
		...
		if (DirtyRegions > 0)
			dynamicCanvas->UpdateTextureRegions((int32)0, DirtyRegions, DirtyUpdateRegions, (uint32)bufferPitch,
			                                    (uint32)bytesPerPixel, canvasPixelData.get());
